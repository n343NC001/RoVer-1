...
async function getNextActivity() {
	...
	switch (currentActivity) {
		...
		case 1: {
			let totalGuilds = (await shardingManager.fetchClientValues(
				'guilds.cache.size'
			)).reduce((prev, val) => prev + val, 0);
			totalGuilds = Util.toHumanReadableNumber(totalGuilds);
			return { text: `over ${totalGuilds} servers`, type: 'WATCHING' };
		}
		case 2:
			return { text: `over ${totalUsers} users`, type: 'WATHCING' };
		case 3:
			return { text: '!help', type: 'LISTENING' };
		case 4:
			return { text: 'Atom Science Verification Software', type: 'STREAMING' };
	}
}
